...
impl Users for UsersImpl {
    ...
    fn get_user_uuid(&self, username: String, password: String) -> Option<String> {
        let user = self.username_to_user.get(&username); // Retrieve `User` or return `None` is user can't be found.
        if let None = user {
            return None;
        }

        let user_uuid = &user.unwrap().user_uuid;
        ...
        if result.is_ok() {
            return Some(user_uuid.to_owned());
        }
